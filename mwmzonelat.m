...
for ff = 1:length(DATA)
	...
	for tt = 1:DATA{ff}.ntrials
		...
		Z = calc_L(DATA{ff}.path(1:DATA{ff}.ntimes(tt),:,tt), platform, optargs.zones);
		if optargs.allquads		
			Z2 = calc_L(DATA{ff}.path(1:DATA{ff}.ntimes(tt),:,tt), platform_q2, optargs.zones);
			Z3 = calc_L(DATA{ff}.path(1:DATA{ff}.ntimes(tt),:,tt), platform_q3, optargs.zones);
			Z4 = calc_L(DATA{ff}.path(1:DATA{ff}.ntimes(tt),:,tt), platform_q4, optargs.zones);
